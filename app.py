...
def execute_pending_generation():
    ...
    try:
        ...
        if source == SOURCE_PDF:
            ...
            spinner_msg = "Generating questions from your study material..."
...
def _render_source_inputs(quiz_source: str):
    """Show topic textarea or PDF uploader based on quiz source."""
    if quiz_source == SOURCE_TOPIC:
        st.markdown("Enter a **topic** or paste **study notes** below.")
        return st.text_area(
            "Topic or study material",
            placeholder="e.g. Python loops\n\nOr paste paragraphs from your textbook/notes...",
            height=180,
...
def _build_generation_params(
    ...
    if quiz_source == SOURCE_TOPIC:
        if not source_text or not source_text.strip():
            st.warning("Please enter a topic or paste study material.")
            return None
